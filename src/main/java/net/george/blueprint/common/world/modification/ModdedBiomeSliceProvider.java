...
public abstract class ModdedBiomeSliceProvider implements DataProvider {
    ...
    /**
     * Constructs a new {@link ModdedBiomeSliceProvider} instance where the {@link #ops} is {@link DynamicRegistryManager#BUILTIN}.
     *
     * @param dataGenerator A {@link DataGenerator} instance to use.
     * @param modid         The ID of the mod to provide data for.
    ...
    protected ModdedBiomeSliceProvider(DataGenerator dataGenerator, String modid) {
        this.dataGenerator = dataGenerator;
        this.modid = modid;
        this.ops = () -> RegistryOps.of(JsonOps.INSTANCE, DynamicRegistryManager.BUILTIN.get());
    }

    @Override
    public void run(DataCache hashCache) {
        HashSet<Identifier> names = new HashSet<>();
        Path outputFolder = this.dataGenerator.getOutput();
        ...
        slices.forEach(pair -> {
            ...
            if (!names.add(name)) {
                ...
                try {
                    DataProvider.writeToPath(GSON, hashCache, slice.serializeWithSelector(pair.getFirst(), ops), path);
    ...
    protected void registerSlice(String name, ConditionedResourceSelector selector, int weight, BiomeUtil.ModdedBiomeProvider provider) {
        this.slices.add(Pair.of(selector, new ModdedBiomeSlice(new Identifier(this.modid, name), weight, provider)));
    }
     ...
     */
    protected final void registerSlice(String name, int weight, BiomeUtil.ModdedBiomeProvider provider, Identifier... levels) {
