...
/**
 * A {@link PillarBlock} extension that fills its item after the latest vanilla stripped wood item.
 */
public class StrippedWoodBlock extends PillarBlock {
    private static final TargetedItemCategoryFiller FILLER = new TargetedItemCategoryFiller(() -> Items.STRIPPED_WARPED_HYPHAE);

    public StrippedWoodBlock(Settings settings) {
        super(settings);
    }
