...
public class EntitySkinHelper<E extends Entity> {
    private final Map<String, Identifier> skins = Maps.newHashMap();
     ...
     * @param path     The path of the folder to look up the textures in.
    ...
    public void putSkins(String skinTexture, String... skinNames) {
        Identifier skinTextureLocation = this.createTextureLocation(skinTexture);
    ...
    private Identifier createTextureLocation(String skin) {
        return new Identifier(this.modId, String.format(this.formattablePath, skin));
    }

    /**
     * Gets the skin texture {@link Identifier} for an entity.
     *
     * @param entity The entity to get the skin for.
     * @return The skin texture {@link Identifier} for an entity or null if no skin could be found for its name.
     */
    @Nullable
    public Identifier getSkinForEntity(E entity) {
        return entity.hasCustomName() ? this.skins.get(entity.getCustomName().getString().toLowerCase().trim()) : null;
    }

    /**
     * Gets the skin texture {@link Identifier} for an entity or defaults to {@code otherSkinTexture} if skin couldn't be looked up.
     *
     * @param entity           The entity to get the skin for.
     * @param otherSkinTexture The fallback texture.
     * @return The skin texture {@link Identifier} for an entity or {@code otherSkinTexture} if null.
     */
    @Nonnull
    public Identifier getSkinForEntityOrElse(E entity, Identifier otherSkinTexture) {
        Identifier skin = this.getSkinForEntity(entity);
